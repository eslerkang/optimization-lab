...
# Hyperparameters 설정
ALPHA_Ss = [0.5]
ALPHA_Es = [0.5, 1.5, 2.5, 10, 25]
BETAs = [0.5, 1.5, 2.5, 10, 25]
SEEDs = [i for i in range(486, 486 + 10)]
Ks = np.arange(0.1, 1.0, 0.1)
NUMS_OF_LOT_PER_PRODUCT = [5, 10, 20]
            ...
            for BETA in BETAs:
                for SEED in SEEDs:
                    for K in Ks:
                        ...
                        types = [1, 2, 3]
                        names = [
                            f"Lot {i}" for i in range(1, NUM_OF_LOT_PER_PRODUCT + 1)
                        ]
                        pairs = [(t, f"{t}-{n}", 0) for t in types for n in names]
                        lot_data = pd.DataFrame(
                            pairs, columns=["type", "name", "release"]
                        ).to_dict("records")
                        ...
                        def get_slack(lot, stage, time):
                            ...
                            # 납기 - 현재 시간 - 남은 작업 시간
                            due_slack = (
                                lot["due"] - time - total_remaining_processing_time
                            )
                        ...
                        def plot_result():
                            ...
                            for lot in lot_data:
                                ...
                                for qtime in lot["qtime"]:
                                    ...
                                    if (
                                        ...
                                        and (q_end_time - base_date).total_seconds()
                                        / 60
                                        > q_due_time
                                    ):
                                ...
                                lot_details.append(
                                    {
                                        ...
                                        / 60,
                                        "End": (end_time - base_date).total_seconds()
                                        / 60,
                                        "Due": due_time,
                                        "Tardiness": tardiness,
                            ...
                            print(f"NUM OF LOT PER PRODUCT: {NUM_OF_LOT_PER_PRODUCT}")
                            ...
                            print(
                                f"Number of q-time violations: {qtime_violations_count}"
                            )
                            print(
                                "========================================================"
                            )
                            results_list.append(
                                {
                                    "NUM OF LOT PER PRODUCT": NUM_OF_LOT_PER_PRODUCT,
                                    "ALPHA_S": ALPHA_S,
                                    "ALPHA_E": ALPHA_E,
                                    "BETA": BETA,
                                    "SEED": SEED,
...
unique_problem_types = df[
    ["NUM OF LOT PER PRODUCT", "ALPHA_S", "ALPHA_E", "BETA"]
].drop_duplicates()
...
for _, problem in unique_problem_types.iterrows():
    # 특정 문제 타입의 데이터 추출
    problem_df = df[
        (df["NUM OF LOT PER PRODUCT"] == problem["NUM OF LOT PER PRODUCT"])
    ...
    for i, metric in enumerate(["MEAN TARDINESS", "TARDY JOB", "Q TARDY JOB"]):
        sns.lineplot(
            data=problem_df, x="K", y=metric, hue="SEED", ax=axs[i], marker="o"
        )

        axs[i].set_title(
            f'{metric} vs K (NUM OF LOT PER PRODUCT={problem["NUM OF LOT PER PRODUCT"]} ALPHA_S={problem["ALPHA_S"]}, ALPHA_E={problem["ALPHA_E"]}, BETA={problem["BETA"]})'
        )
        axs[i].set_xlabel("K")
    ...
    # 그래프 저장
    image_file = f'./problem__{problem["NUM OF LOT PER PRODUCT"]}_{problem["ALPHA_S"]}_{problem["ALPHA_E"]}_{problem["BETA"]}.png'
