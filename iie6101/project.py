    ...
    for ALPHA_S in ALPHA_Ss:
        ...
        for SEED in SEEDs:
            for K in Ks:
                ...
                # Data 불러오기 및 설정
                types = [1, 2, 3]
                names = [f"Lot {i}" for i in range(1, NUM_OF_LOT_PER_PRODUCT + 1)]
                ...
                def generate_queue_time():
                    for q in q_time:
                        ...
                        for i in range(q["start"], q["end"]):
                            total_processing_time_in_q_time_range += processing_times[
                                i + 1
                            ]
                        q["limit"] = total_processing_time_in_q_time_range + np.rint(
                            np.random.uniform(
                                0,
                                BETA * total_processing_time_in_q_time_range,
                            )
                        )
                ...
                def plot_result():
                    ...
                    for lot in lot_data:
                        ...
                        for qtime in lot["qtime"]:
                            ...
                            q_end_time = df[
                                (df["Lot"] == lot_id) & (df["Stage"] == q_end_stage + 1)
                            ]["Start"].min()
                    ...
                    fig_table = go.Figure(
                        data=[
                            go.Table(
                                ...
                                cells=dict(
                                    values=[
                                        lot_info_df[k].apply(str)
                                        for k in lot_info_df.columns
                                    ],
                                    fill_color="lavender",
                                    align="left",
                                ),
                    ...
                    print(f"Mean tardiness of lots: {total_tardiness / len(lot_data)}")
